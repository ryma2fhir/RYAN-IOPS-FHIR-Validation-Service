...
class CodingSupport(@Qualifier("R4") private val ctx: FhirContext?,
    ...
    fun lookupCode(code :String) : IValidationSupport.LookupCodeResult? {
        var lookupCodeResultUK = cacheCoding.getIfPresent(uk.nhs.england.fhirvalidator.util.FhirSystems.SNOMED_CT + "-"+ code)
        if (lookupCodeResultUK != null) return lookupCodeResultUK
        lookupCodeResultUK = supportChain.lookupCode(this.validationSupportContext,  uk.nhs.england.fhirvalidator.util.FhirSystems.SNOMED_CT, code) as uk.nhs.england.fhirvalidator.shared.LookupCodeResultUK
        if (lookupCodeResultUK!=null) {
            cacheCoding.put(uk.nhs.england.fhirvalidator.util.FhirSystems.SNOMED_CT + "-"+ code,lookupCodeResultUK)
    ...
    fun getTypeCoding(lookupCodeResult: IValidationSupport.LookupCodeResult) : Coding? {
        if (lookupCodeResult is uk.nhs.england.fhirvalidator.shared.LookupCodeResultUK) {
            var lookupCodeResultUK = lookupCodeResult as uk.nhs.england.fhirvalidator.shared.LookupCodeResultUK
                    ...
                    if (property.part[0].name.equals("code")) {
                        if (property.part[0].value is CodeType) {
                            ...
                            if ((valueType.value.equals("parent") || valueType.value.equals("child")) && property.part[1].value is CodeType) {
                                ...
                                when (code.value) {
                                    "10363801000001108" -> {
                                        return Coding().setSystem(uk.nhs.england.fhirvalidator.util.FhirSystems.SNOMED_CT).setCode(code.value).setDisplay("Virtual medicinal product")
                                    }
                                    "10363901000001102" -> {
                                        return Coding().setSystem(uk.nhs.england.fhirvalidator.util.FhirSystems.SNOMED_CT).setCode(code.value).setDisplay("Actual medicinal product")
                                    }
                                    "10364001000001104" -> {

                                        return Coding().setSystem(uk.nhs.england.fhirvalidator.util.FhirSystems.SNOMED_CT).setCode(code.value).setDisplay("Actual medicinal product pack")

                                    }
                                    "8653601000001108" -> {

                                        return Coding().setSystem(uk.nhs.england.fhirvalidator.util.FhirSystems.SNOMED_CT).setCode(code.value).setDisplay("Virtual medicinal product pack")
    ...
    fun getCodeableConcept(code :String) : CodeableConcept? {
        ...
        return CodeableConcept().addCoding(Coding()
            .setSystem(uk.nhs.england.fhirvalidator.util.FhirSystems.SNOMED_CT)
            .setCode(code)
